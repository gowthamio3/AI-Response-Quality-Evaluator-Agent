...
st.title("🤖 Development of AI Response Validation System with Hallucination Detection Assistance Group1")
...
if st.session_state.get("single_evaluation_done", False):
    ...
    if normalized_question in AI_RESPONSES:
        ...
        st.header("🤖 Compare Two AI Systems")

        st.write(
            "Select an AI system to automatically evaluate its response "
        ...
        if st.session_state.get("copilot_result"):

            copilot = st.session_state["copilot_result"]

            st.divider()

            st.subheader("🟦 Copilot Evaluation")
            ...
            st.text(copilot["relevance"])
            st.text(copilot["accuracy"])
            st.text(copilot["hallucination"])
            st.text(copilot["completeness"])
        ...
        if st.session_state.get("gemini_result"):
            ...
            gemini = st.session_state["gemini_result"]
            ...
            st.text(gemini["relevance"])
            st.text(gemini["accuracy"])
            st.text(gemini["hallucination"])
            st.text(gemini["completeness"])
        ...
        if (
            ...
            and
            st.session_state.get("gemini_result")
        ):

            st.divider()

            st.header("📊 Copilot vs Gemini Comparison")
            ...
            st.success(
                "✅ Both AI systems have been evaluated using the same "
        ...
        st.warning(
            "⚠️ Two-AI comparison is currently available for the "
            "questions stored in the comparison dataset."
        )
...
st.caption(
    "Developed by Sarvasuddi Gowthami | Infosys Virtual Internship 7.0"
)